...
let rec ask_for_another_game () =
	print_endline "Do you want to play again? (y/n)";
	let input = (read_line ()) in

	let parse_input str =
		if String.equal str "y" || String.equal str "yes" then
			(true, true)
		else if String.equal str "n" || String.equal str "no" then
			(true, false)
		else
			(false, false)
	in

	let (valid, answer) = (parse_input input) in
